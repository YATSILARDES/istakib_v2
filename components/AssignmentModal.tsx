...
const AssignmentModal: React.FC<AssignmentModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
      <div className="bg-white border border-slate-200 w-full max-w-6xl rounded-xl shadow-2xl overflow-hidden flex flex-col h-[95dvh] md:h-[90vh]">

        {/* Başlık */}
        <div className="relative px-4 py-3 md:px-6 md:py-4 border-b border-slate-200 flex flex-col md:flex-row items-start md:items-center justify-between gap-4 bg-slate-50">
          <div className="flex flex-col md:flex-row items-start md:items-center gap-4 w-full md:w-auto pr-8 md:pr-0">
            ...
            <div className="flex items-center gap-2">
              ...
              {showAddStaffInput ? (
                <div className="flex items-center gap-2 animate-fadeIn bg-white p-1 rounded-lg border border-slate-200 shadow-sm">
                  <div className="flex flex-col gap-1">
                    <input
                      ...
                      className="bg-white border border-slate-300 text-slate-800 text-xs rounded p-1.5 outline-none w-40 focus:ring-1 focus:ring-blue-500"
                    />
                    <input
                      type="email"
                ...
                <div className="flex items-center gap-2">
                  <select
                    ...
                    className="bg-white border border-slate-300 text-slate-800 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block p-2 outline-none min-w-[200px]"
                  >
            ...
            {selectedStaffEmail && (
              <span className="text-xs text-slate-500 bg-slate-100 px-2 py-1 rounded border border-slate-200 hidden md:block">
                {selectedStaffEmail}
              </span>
            )}
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              onClick={handlePrint}
              className="bg-white border border-slate-300 hover:bg-slate-50 text-slate-700 px-3 py-2 rounded-lg flex items-center gap-2 text-sm transition-colors shadow-sm"
            >
              <Printer className="w-4 h-4" /> Yazdır
            </button>
            <button onClick={onClose} className="text-slate-400 hover:text-slate-600 p-2 transition-colors absolute top-2 right-2 md:static">
              <X className="w-6 h-6" />
            </button>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-hidden grid grid-cols-1 md:grid-cols-2 divide-y md:divide-y-0 md:divide-x divide-slate-200">
          ...
          <div className="flex flex-col bg-slate-50 min-h-0">
            <div
              className="p-3 bg-white border-b border-slate-200 font-medium text-slate-700 flex justify-between cursor-pointer md:cursor-default"
            ...
            <div className={`flex-1 overflow-y-auto p-4 space-y-6 custom-scrollbar pb-24 ${!isPoolSectionExpanded ? 'hidden md:block' : ''}`}>
              ...
              <div className="mb-8 border border-slate-200 rounded-lg overflow-hidden md:border-none md:rounded-none">
                <div
                  className="sticky top-0 bg-slate-50 z-10 pb-2 pt-1 mb-2 border-b border-slate-200 cursor-pointer select-none"
                      ...
                      {['Tümü', ...Array.from(new Set(unassignedTasks.map(t => t.district).filter(Boolean)))].sort().map(dist => (
                        <button
                          ...
                          className={`text-[10px] px-2.5 py-1 rounded-full whitespace-nowrap transition-colors border ${activeMainDistrict === (dist || 'Tümü')
                            ...
                            : 'bg-white text-slate-600 border-slate-200 hover:border-blue-400 shadow-sm'
                            }`}
                        >
                          {dist || 'Belirtilmemiş'}
                        </button>
                  ...
                  <div className="space-y-2 px-2 pb-2">
                    {filteredMainTasks.length === 0 ? (
                      ...
                      filteredMainTasks.map(task => (
                        <div key={task.id} className="bg-white border border-slate-200 rounded-lg p-3 flex items-center justify-between group hover:border-blue-300 transition-colors shadow-sm">
                          <div>
              ...
              {/* Atanmamış Eksikler */}
              <div className="mb-4 border border-slate-200 rounded-lg overflow-hidden md:border-none md:rounded-none">
                <div
                  className="sticky top-0 bg-slate-50 z-10 pb-2 pt-1 mb-2 border-b border-slate-200 cursor-pointer select-none"
                      ...
                      {['Tümü', ...Array.from(new Set(unassignedRoutineTasks.map(t => t.district).filter(Boolean)))].sort().map(dist => (
                        <button
                          ...
                          className={`text-[10px] px-2.5 py-1 rounded-full whitespace-nowrap transition-colors border ${activeRoutineDistrict === (dist || 'Tümü')
                            ...
                            : 'bg-white text-slate-600 border-slate-200 hover:border-purple-400 shadow-sm'
                            }`}
                        >
                          {dist || 'Belirtilmemiş'}
                        </button>
                  ...
                  <div className="space-y-2 px-2 pb-2">
                    {filteredRoutineTasks.length === 0 ? (
                      ...
                      filteredRoutineTasks.map(task => (
                        <div key={task.id} className="bg-white border border-slate-200 rounded-lg p-3 flex items-start justify-between group hover:border-purple-300 transition-colors shadow-sm">
                          <div className="pr-2 flex-1 overflow-hidden">
                            ...
                            {(task.customerName || task.phoneNumber || task.address || task.district) && (
                              <div className="flex flex-wrap items-center gap-x-2 gap-y-1 mb-1.5 text-xs">
                                {task.district && (
                                  <span className="text-purple-600 bg-purple-100 px-1.5 py-0.5 rounded border border-purple-200 flex items-center gap-1">
          ...
          <div className="flex flex-col bg-slate-50 min-h-0 border-l border-slate-200">
            <div className="flex items-center justify-between p-3 bg-white border-b border-slate-200">
              <div
                className="font-medium text-blue-600 flex items-center gap-2 cursor-pointer md:cursor-default"
            ...
            <div className={`flex-1 overflow-y-auto p-4 custom-scrollbar print:overflow-visible ${!isStaffListExpanded ? 'hidden md:block' : ''}`}>
              ...
              {viewMode === 'list' ? (
                  ...
                  <div>
                    ...
                    <div className="space-y-2">
                      ...
                      {staffTasks.filter(task => {
                        ...
                        <div key={task.id} className="bg-white border border-slate-200 rounded-lg p-3 flex items-center justify-between group hover:border-red-400 transition-colors shadow-sm">
                          <button
                            onClick={() => onAssignTask(task.id, '', undefined)}
                            className="bg-slate-100 hover:bg-red-500 hover:text-white text-slate-400 p-1.5 rounded-md transition-colors mr-3"
                  ...
                  <div>
                    ...
                    <div className="space-y-2">
                      ...
                      {staffRoutineTasks.filter(task => {
                        ...
                        <div key={task.id} className="bg-white border border-slate-200 rounded-lg p-3 flex items-start justify-between group hover:border-red-400 transition-colors shadow-sm">
                          <button
                            onClick={() => onAssignRoutineTask(task.id, '', undefined)}
                            className="bg-slate-100 hover:bg-red-500 hover:text-white text-slate-400 p-1.5 rounded-md transition-colors mr-3 flex-shrink-0 mt-1"
                            title="Havuz'a Gönder (Atamayı Kaldır)"
                          >
                            <ArrowLeft className="w-4 h-4" />
                          </button>
                          <div className="flex-1 overflow-hidden">
                            ...
                            <div className="text-sm text-slate-600 break-all whitespace-pre-wrap">{task.content}</div>
                            <div className="mt-1 flex justify-end">
                              <span className="text-[10px] text-slate-500 bg-slate-900/50 px-1.5 py-0.5 rounded border border-slate-700/50">
                                {new Date(task.createdAt?.seconds ? task.createdAt.seconds * 1000 : task.createdAt).toLocaleDateString('tr-TR')}
                              </span>
                ...
                /* WEEKLY VIEW */
                <div className="grid grid-cols-7 h-full min-w-[800px] gap-1 bg-slate-50 rounded-lg p-1 border border-slate-200">
                  {/* Render 7 Columns */}
                  {Array.from({ length: 7 }).map((_, i) => {
                    ...
                    return (
                      <div
                        ...
                        className={`flex flex-col border-r border-slate-200 last:border-0 transition-colors cursor-pointer
                           ${isToday ? 'bg-blue-50' : ''}
                           ${selectedDate && selectedDate.toDateString() === dayDate.toDateString() ? 'bg-blue-100 ring-1 ring-inset ring-blue-400' : 'hover:bg-slate-100'}
                         `}
                      >
                        {/* Column Header */}
                        <div className={`text-center py-2 border-b border-slate-200 ${isToday ? 'text-blue-600 font-bold' : 'text-slate-500'}`}>
                        ...
                        <div className="flex-1 p-1 space-y-1 overflow-y-auto">
                          ...
                          {dayRoutineTasks.map(t => (
                            <div key={t.id} className="bg-white border border-slate-200 rounded p-1.5 text-[10px] group relative hover:z-10 hover:shadow-lg transition-all shadow-sm">
                              {t.customerName && <div className="font-bold text-blue-600 mb-0.5 truncate">{t.customerName}</div>}
                              <div className="line-clamp-3 text-slate-600 mb-1">{t.content}</div>
                              <div className="flex justify-between items-center opacity-60">
                                <span className="text-[9px] text-purple-600">Eksik</span>
                                <button
                                  onClick={(e) => {
                          ...
                          {dayMainTasks.map(t => (
                            <div key={t.id} className="bg-blue-50 border border-blue-200 rounded p-1.5 text-[10px] group relative hover:z-10 hover:shadow-lg transition-all shadow-sm">
                              <div className="font-semibold text-blue-800 mb-0.5 truncate">{t.title}</div>
                              <div className="line-clamp-2 text-slate-600 mb-1">{t.address}</div>
                              <div className="flex justify-between items-center opacity-60">
                                <span className="text-[9px] text-blue-600">Saha</span>
                                <button
                                  onClick={(e) => {
                                    e.stopPropagation();
