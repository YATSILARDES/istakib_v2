...
const FieldStaffStatusModal: React.FC<FieldStaffModalProps> = ({
    ...
    const getWeeklyData = (staffEmail: string) => {
        ...
        // Merge lists for easier sorting
        const combined = [
            ...tasks.filter(t => {
                const isTaskActive = t.status !== TaskStatus.CHECK_COMPLETED && t.status !== TaskStatus.DEPOSIT_PAID;
        ...
        combined.forEach(item => {
            ...
            if (dayObj) {
                ...
                // Add to a generic 'items' list on dayObj if we want mixed rendering later
                // For now, we keep separate lists 'tasks' and 'routine' on dayObj to respect current rendering
                // BUT current rendering in FieldStaffStatusModal separates them visually (Routine vs Main).
                // If we want MIXED rendering in this modal too, we need to change the renderer.
                // The user asked for "kullanıcıların kendi saha görevlerini ve eksikleri gördüğü liste" -> Mobile.
                // But "FieldStaffStatusModal" is the admin view of that same user.
                // To be consistent, we SHOULD show them mixed here too? 
                // Let's first SORT them within their categories correctly at least.
            }
        });

        // Sort lists within days
        days.forEach(day => {
            day.tasks.sort((a, b) => (a.dailyOrder || 0) - (b.dailyOrder || 0));
            day.routine.sort((a, b) => (a.dailyOrder || 0) - (b.dailyOrder || 0));

            // Note: Since they are rendered separately in this modal (lines 260-300), 
            // cross-list sorting doesn't affect visual order unless we merge the rendering loop too.
            // Given the user request focused on "their own list" (Mobile), 
            // I will keep this modal's structure but ensure each list is internally sorted by order.
        });
