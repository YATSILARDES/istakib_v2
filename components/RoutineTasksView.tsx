...
const RoutineTasksView: React.FC<RoutineTasksViewProps> = ({
    ...
    const renderTaskCard = (task: RoutineTask, isCompletedView: boolean) => (
        <div key={task.id} className={`group flex items-start gap-4 p-4 rounded-xl border transition-all animate-in fade-in slide-in-from-bottom-2 ${isCompletedView ? 'bg-slate-50 border-slate-100 opacity-60' : 'bg-white border-slate-100 hover:border-purple-300 hover:shadow-md'} ${editingTaskId === task.id ? 'ring-2 ring-purple-500/50 border-purple-500/50' : ''}`}>
            <button
            ...
            <div className="flex-1 min-w-0">
                {(task.customerName || task.phoneNumber || task.address || task.district) && (
                    <div className="flex flex-wrap items-center gap-x-4 gap-y-2 mb-2 text-sm">
                        {task.customerName && (
                            <span className={`font-semibold flex items-center gap-1.5 ${isCompletedView ? 'text-slate-600' : 'text-blue-600'}`}>
                        ...
                        {task.district && (
                            <span className="bg-purple-50 text-purple-700 px-2 py-0.5 rounded-full text-xs font-medium border border-purple-100">
                                {task.district}
                            </span>
                        )}
                        {task.address && (
                ...
                <p className={`${isCompletedView ? 'text-slate-500 line-through' : 'text-slate-700'} whitespace-pre-wrap break-words leading-relaxed`}>{task.content}</p>
    ...
    return (
        <div className="flex flex-col h-full bg-slate-50 overflow-hidden animate-in fade-in duration-300">
            ...
            <div className="flex-1 flex flex-col lg:flex-row overflow-hidden">
                ...
                <div className={`bg-white border-r border-slate-200 flex-shrink-0 lg:w-[400px] flex flex-col transition-all duration-300 z-20 shadow-xl lg:shadow-none ${isAddFormExpanded ? 'w-full lg:w-[400px]' : 'h-14 lg:h-auto overflow-hidden'}`}>
                    <div
                ...
                {/* RIGHT: LIST */}
                <div className="flex-1 bg-slate-100/50 flex flex-col min-h-0">
