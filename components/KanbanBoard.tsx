  ...
  return (
    <div className="flex-1 overflow-x-auto overflow-y-hidden p-6">
      <div className="flex gap-6 h-full min-w-[1500px]">
        {showRoutineColumn && (
          <div className="flex-1 flex flex-col min-w-[280px] bg-slate-800/40 rounded-2xl border border-white/5 backdrop-blur-sm self-start max-h-full">
            {/* Header */}
            <div className="p-4 border-b border-white/5 bg-purple-500/5 flex items-center justify-between rounded-t-2xl">
              <div className="flex items-center gap-2 font-semibold text-purple-100">
                ...
                <span className="ml-2 px-2 py-0.5 text-xs bg-purple-500/10 border border-purple-500/20 rounded-full text-purple-200">
            ...
            <div className="px-3 py-2 border-b border-white/5">
              <div className="relative group">
                ...
                <input
                  ...
                  className="w-full bg-slate-900/50 border border-white/5 rounded-xl py-1.5 pl-8 pr-3 text-xs text-slate-200 placeholder-slate-600 focus:outline-none focus:border-purple-500/30 focus:ring-1 focus:ring-purple-500/20 transition-all"
                />
              </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-3 space-y-3 custom-scrollbar">
                ...
                <div className="mb-4 space-y-3">
                  ...
                  {filteredStandard.map(t => (
                    <div
                      ...
                      className={`
                        p-3.5 rounded-xl border transition-all cursor-pointer group shadow-lg shadow-black/20 hover:-translate-y-0.5
                        ${t.checkStatus === 'missing'
                          ...
                          : t.checkStatus === 'clean'
                            ...
                            : 'bg-slate-800/90 border-l-4 border-l-blue-500 border-white/5 hover:border-blue-500/50'
                        }
                      `}
                    >
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="text-sm text-slate-200 font-medium leading-snug group-hover:text-blue-300 transition-colors">{t.title}</div>
                          {t.address && (
                            <div className="flex items-center gap-1.5 mt-1.5 text-xs text-slate-400 truncate">
                              <MapPin className="w-3 h-3 flex-shrink-0" />
                              {t.address}
                            </div>
                          )}
                          <div className="mt-2 flex items-center justify-between">
                            <span className="text-[10px] text-slate-500 font-mono bg-slate-900/50 px-1.5 py-0.5 rounded">#{t.orderNumber}</span>
                            <span className="text-[10px] text-blue-400 font-medium">{StatusLabels[t.status]}</span>
                          </div>
                        </div>
                      </div>
                    </div>
              ...
              {filteredRoutine.length > 0 && (
                <div className="space-y-3">
                  {filteredStandard.length > 0 && <div className="text-[10px] font-bold text-purple-400 uppercase tracking-widest pl-1 pt-2 border-t border-white/5">Eksikler / Notlar ({filteredRoutine.length})</div>}
                  {filteredRoutine.map(t => (
                    <div key={t.id} className={`flex flex-col gap-2 p-3.5 rounded-xl border transition-all shadow-lg shadow-black/10 hover:-translate-y-0.5 ${t.isCompleted
                      ? 'bg-slate-900/40 border-slate-800 text-slate-500 opacity-60'
                      : 'bg-slate-800/90 border-white/5 hover:border-purple-500/30'
                      }`}>
                      <div className="flex items-start gap-3">
                        <button
        ...
        {columns.map((status) => {
          ...
          return (
            <div key={status} className="flex-1 flex flex-col min-w-[280px] bg-slate-800/40 rounded-2xl border border-white/5 backdrop-blur-sm">
              {/* Column Header */}
              <div className="p-4 border-b border-white/5 flex items-center justify-between">
                <div className="flex items-center gap-2 font-semibold text-slate-200">
                  <StatusIcon status={status} />
                  <span className="truncate">{StatusLabels[status]}</span>
                  <span className="ml-2 px-2 py-0.5 text-xs bg-slate-700/50 border border-slate-600/50 rounded-full text-slate-300">
              ...
              <div className="px-3 py-2 border-b border-white/5">
                <div className="relative group">
                  ...
                  <input
                    ...
                    className="w-full bg-slate-900/50 border border-white/5 rounded-xl py-1.5 pl-8 pr-3 text-xs text-slate-200 placeholder-slate-600 focus:outline-none focus:border-blue-500/30 focus:ring-1 focus:ring-blue-500/20 transition-all"
                  />
                </div>
              </div>
              ...
              <div className="flex-1 overflow-y-auto p-3 space-y-3 custom-scrollbar">
                {filteredTasks.map(task => (
                  <div
                    ...
                    className={`
                      px-3.5 py-3.5 rounded-xl border transition-all cursor-pointer group relative shadow-lg shadow-black/20 hover:-translate-y-0.5
